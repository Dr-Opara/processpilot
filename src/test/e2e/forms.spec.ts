...
test.describe("Request demo form", () => {
  test("shows validation errors on empty submit, then succeeds in dev mode", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/development mode: this request was validated/i),
    ).toBeVisible();
  });
});

test.describe("Start trial form", () => {
  test("rejects a weak password and requires terms agreement", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/development mode: this signup was validated/i),
    ).toBeVisible();
  });
});
