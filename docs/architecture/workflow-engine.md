...
4. **Deadline tracking** — the `workflow-deadline-check` background job
   ...
   `workflow.deadline_breached` history/audit event. Full escalation
   (reminders, reassignment, manager/process-owner escalation) is Phase
   10's job, layered on top of this detection.
...
| `form`/`evidence` | Execute identically to `human_task` this phase. Structured, validated field capture and evidence upload are Phase 9's job, layered on `tasks.output` later.                                                                                                                         |
| `approval`        | A single assignee decision (`decideApproval()` — approve/reject), not Phase 10's configurable multi-step chain. A rejected _required_ approval fails the whole workflow rather than routing down an unspecified rejection branch.                                                   |
