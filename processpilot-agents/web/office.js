...
function renderReviews(){const pending=reviews();if(!pending.length)return '<p class="empty">'+(activity?.reviewsCheckedAt?'No pending review requests in the latest recorded mailbox check.':'Review status has not been verified. Waiting for a recorded mailbox check.')+'</p>';
...
function renderPanel(){
 ...
 ['Gmail','Used for replies, duplicate checks, permitted outreach and review notifications when connected. Mailbox last checked: '+stamp(activity?.reviewsCheckedAt)+'.'],
 ['GitHub','Stores source code and encrypted activity records. The connection indicator shows whether this office can retrieve updates; it checks every 30 seconds.'],
 ...
 ['Hourly ChatGPT workflow','Scheduled hourly, 8 AM–4 PM Central. These are coordinated roles in a scheduled workflow; the latest records show which roles actually ran.']
...
function applyActivity(){
 ...
 $('#reviewBadge').textContent=pending?pending+' review'+(pending>1?'s':''):activity.reviewSyncWarning?'Review sync delayed':activity.reviewsCheckedAt?'No pending reviews':'Mailbox not checked';$('#me').classList.toggle('alert',pending>0);$('#tabCount').textContent=pending||'';
...
async function refresh(){if(loading)return;loading=true;$('#refresh').disabled=true;try{const res=await fetch('/api/activity',{cache:'no-store'});if(!res.ok)throw Error('Live feed unavailable');const data=await res.json();if(data.schemaVersion!==1||!Array.isArray(data.agents)||!Array.isArray(data.reviews))throw Error('Invalid activity update');activity=data;const age=Date.now()-Date.parse(data.updatedAt);$('#connection').textContent=age>90*60000?'Connected · last record is over 90 minutes old':'Connected · recorded activity';$('#connection').className='connection'+(age>90*60000?' error':'');$('#updated').textContent='Last update '+stamp(data.updatedAt);applyActivity();}catch{$('#connection').textContent='Connection unavailable · results may be stale';$('#connection').className='connection error';$('#reviewBadge').textContent=activity?(reviews().length?reviews().length+' reviews · sync delayed':'Review status may be stale'):'Status unavailable';renderPanel();}finally{loading=false;$('#refresh').disabled=false;}}
function updateClock(){const now=new Date();$('#clock').textContent=now.toLocaleTimeString('en-US',{timeZone:'America/Chicago',hour:'numeric',minute:'2-digit'})+' CT';const h=Number(new Intl.DateTimeFormat('en-US',{timeZone:'America/Chicago',hour:'numeric',hourCycle:'h23'}).format(now));const next=h<8?8:h<16?h+1:null;$('#nextRun').textContent='Scheduled: '+(next===null?'tomorrow 8 AM':(next>12?next-12:next)+(next>=12?' PM':' AM'))+' CT';}
