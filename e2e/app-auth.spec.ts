 ...
 * which skips Clerk setup entirely, only failing these two tests, if
 * CLERK_SECRET_KEY isn't set) and one pre-existing test user. Create it
 * once in the Clerk dashboard using a "+clerk_test@" email (e.g.
 * e2e+clerk_test@processpilot.dev) — Clerk's test mode accepts the fixed
 * code 424242 for any such address, so no real inbox is needed.
 */
